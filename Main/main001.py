...
def janela_editar_rota(): # Janela para editar rota
    ...
    def editar(): # Funcao criada para fazer com que seja possivel pegar os parametros e usar na interface
        try:
            Conexao.editar_rota(linhas, linha_original.get(), cidade_origem.get(), cidade_destino.get()) # Joga na funcao de editar rota
        except ValueError: # Erro de tipo de dado
    ...
    janela_botoes.pack(pady=3) # Lugar de botoes
    
    btn_editar = ctk.CTkButton(janela_botoes,text="Editar", command=editar)
    btn_editar.pack(side="left", padx=3) # Quando clicado ele retorna a funcao de editar
 
    btn_voltar = ctk.CTkButton(janela_botoes, text="Voltar", command=janela_editar_rota.destroy, fg_color="#4682B4", text_color="white")
    btn_voltar.pack(side="left", padx=3) # Quando clicado destroi a janela atual

def janela_editar_horario(): # Janela para editar horario
    janela_editar_horario = ctk.CTkToplevel(janela) # Janela secundaria, nivel filha de janela
    janela_editar_horario.title("Editar Horario") # Titulo da janela
    janela_editar_horario.geometry("500x600") # Tamanho da janela
    ...
    linha_original.pack(pady=2) # Lugar que recolhe a linha que deseja editar
# Codigo para rodar  python3 -m Main.main001
