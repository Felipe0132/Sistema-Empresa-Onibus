...
def janela_adicionar_onibus(): # Funcao para criar a janela responsavel para recolher informacoes de adicionar linha
    janela_adicionar_onibus = ctk.CTkToplevel(janela)
    janela_adicionar_onibus.title("Adicionando Onibus")
    janela_adicionar_onibus.geometry("500x690")
    janela_adicionar_onibus.configure(fg_color='white')

    titulo = ctk.CTkLabel(janela_adicionar_onibus, text="Adicionar dados", text_color='black', font=('Arial', 25))
    titulo.pack(pady=10)

    try:
        horario_saida = ctk.CTkEntry(janela_adicionar_onibus, placeholder_text="Digite o Horario de saida...", width=320, height=50)
        horario_saida.pack(pady=10)

        linha = ctk.CTkEntry(janela_adicionar_onibus, placeholder_text="Digite a linha que deseja adicionar...", width=320, height=50)
        linha.pack(pady=10)
        ...
        for linha in linhas_disponiveis:
            ctk.CTkLabel(scrolllist, text=linha).pack(pady=2) # Aqui ele coloca na caixa de scroll os itens da lista

        # Cria o campo dos botões abaixo, para não conflitar com o espaço
        janela_botoes = ctk.CTkFrame(janela_adicionar_onibus, fg_color="white")
        janela_botoes.pack(pady=5)

        verificacao = Conexao.adicionar_onibus(linhas, Onibus(datetime.strptime(horario_saida.get(), linha.get(), "%d/%m/%Y %H:%M"))) # Aqui retorna algo para o exception olhar
        # Nessa linha acima eh usado .get() para conseguir converter o valores das caixas
    except Exception as e:
    ...
    btn_inserir = ctk.CTkButton(janela_botoes,text="Inserir", command=lambda: Conexao.adicionar_onibus(linhas, linha, Onibus(datetime.strptime(horario_saida.get(), "%d/%m/%Y %H:%M")))) #TODO
